      ...
      <div className={`data-actions-wrapper ${selectedModeClass}`}>
        <div className="flex btn-bar">
          <button className=" btn-md btn-neutral relative">
            ...
            <span>
              {' '}
              <span className="count">{selectedCount.current}</span> selected
            </span>
          </button>
            ...
            {data.map((dataEntity) => {
              return (
                <DataPreview
                  ...
                  type={type}
                  key={dataEntity._id}
                  isSelected={checkIsSelected(dataEntity)}
                  handleChange={() => handleSelection(dataEntity)}
                />
              );
            })}
